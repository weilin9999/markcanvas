...
public class DrawCircle {
    /**
     * canvasDrawCircleElement Json数据转换成为Canvas画布实体 绘制圆形
     * 2023-04-30
     * @param canvas 画布
     * @param object Json数据 type_id = "circle" 的类型数据）
     ...
     */
    @NonNull
    public static PositionEntity canvasDrawCircleElement(@NonNull Canvas canvas, @NonNull JSONObject object,
        ...
        float size = object.getIntValue(("size"));
        float size_percent = object.getFloatValue(("size_percent"));
        ...
        if (size_percent > 0) {
            size = (CanvasUtils.retPercentWidth(canvasWidth, (float) (size_percent * 0.01)) + size) / 2;
        } else {
            if (size == -1) {
                size = (float) (canvasWidth / 2);
            }
        }
        ...
        if (align_left != null && align_left.size() != 0){
            ...
            else if (Objects.equals(align_left_mode, ("center"))) left = max_left + ((max_right - max_left)/2) - size + left;
        ...
        if (align_top != null && align_top.size() != 0){
            ...
            else if (Objects.equals(align_top_mode, ("center"))) top = min_top + ((max_bottom - min_top)/2) - size + top;
        ...
        if (!Objects.equals(margin_left_item, (""))){
            if (Objects.equals(margin_left_item, ("self"))){
                offset_left = CanvasUtils.retPercentWidth((int) (size * 2), (float) (margin_left_percent * 0.01)) + offset_left;
            }else {
        ...
        if (!Objects.equals(margin_top_item, (""))){
            if (Objects.equals(margin_top_item, ("self"))){
                offset_top = CanvasUtils.retPercentHeight((int) (size * 2), (float) (margin_top_percent * 0.01)) + offset_top;
            }else {
        ...
        if (!isVirtual){
            ...
            if (!Objects.equals(mask_item, (""))){
                ...
                canvas.drawCircle((left + offset_left), (top + offset_top), size, mPaint);
    ...
    /**
     * canvasDrawCircleMaskElement 圆形的蒙版
     * 2023-05-02
     * @param canvas 画布
     * @param object Json数据 type_id = "circle" 的类型数据）
     * @param canvasWidth 画布宽度
     * @param canvasHeight 画布和高度
     */
    public static void canvasDrawCircleMaskElement(@NonNull Canvas canvas, @NonNull JSONObject object,
